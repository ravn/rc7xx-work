...
(2) **#89 investigation.**  Ruled out Path 1 (remove `isAsCheapAsAMove` from `LD_r16_nn` pseudo) empirically.  The change correctly hoists the loop-invariant `LD rr,nn` out of loop bodies via MachineLICM (good for #89's reproducer — 24 B → 23 B; constant moves to entry block) BUT regresses real workloads by **+15 B BIOS / +20 B cpnos-rom** because the same flag controls regalloc remat, and remat-at-use-site is the right call in 9 of 11 measured sites (`_specc`, `_bios_conin`, `_bg_clear_from`, `_erase_to_eol`, etc.).  TableGen flag is too coarse — `isAsCheapAsAMove` couples MachineLICM hoisting and regalloc remat, and Z80 needs them to behave oppositely.  Fix needs pass-level intervention, not TableGen surgery.  Reverted; no commit landed for the change itself (only the investigation doc).  Findings in `tasks/issue-89-investigation-2026-05-03.md` with four next-step options ranked.  Lessons: (a) TableGen flags couple multiple passes — when the target needs them to diverge, the flag is the wrong knob; (b) synthetic-only data misled — the synthetic showed the structural fix "working" with the bottleneck merely shifting, but real-workload measurement surfaced the regression in 9 functions; always measure rcbios + cpnos-rom before believing a regalloc-area change.

**Net session 42 status:** Phase 2 closed (admin); Phase 3 entry #89 first-session deliverable is an investigation doc (no fix attempt), not a fix.  Next session on #89 should produce a design doc choosing among the four options in `issue-89-investigation-2026-05-03.md`; most likely option (c): merge #89 into the broader #94/#98/#27 cost-model surface.
