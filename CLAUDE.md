...
Session #48 (2026-05-09): cpnos-rom SDCC pio-irq reaches PASS — z88dk-zsdcc compiler bug found and worked around (Phase 48), then dead Python netboot harness retired (Phase 48b).

(1) **z88dk-zsdcc 4.5.0 constant-folding bug** in cpnos-rom/cfgtbl.c.  The `(NET_DRV(letter, 0x00) >> 8) & 0xFF` pattern should evaluate to 0x00 (NET_DRV produces `(uint16_t)0x0080`; `>> 8` is 0).  SDCC emits 0xFF instead — sign-extends the 0x80 literal, then arithmetic-shifts.  Result: every CP/NET drive entry's server-slave-id byte is 0xFF (no master); NDOS sees no valid CP/NET server, falls through to local BDOS, never sends SNDMSG/RCVMSG, slave warm-boots in a tight cycle (the symptom previously tracked as #60 "control-flow lost ~3.6s after NDOSE handoff").  Clang LLVM Z80 compiles the same C source correctly.  Filed as **ravn/z88dk#4**.

(2) **Workaround applied** in `rc700-gensmedet@1641749` (Phase 48): rewrote `cfgtbl_init_template` with explicit byte literals `{0x80, 0x00, 0x81, 0x00, ...}` instead of the macro form.  Identical semantics, sidesteps the buggy `>> 8` constant-folding path.  Tracking issue **ravn/rc700-gensmedet#67** for reverting to the macro form once z88dk#4 is fixed upstream.

(3) **Diagnostic that found it**: `cpnos-rom/mame_minimal_trace.lua` (added in Phase 48).  Taps BDOS entry + NDOS dispatchers (NDOSA/NDOSE) + SNDMSG/RCVMSG body addresses with retaddr filtering to focus on NDOS-side activity.  Showed under SDCC: 202 BDOS calls, 0 SNDMSG/RCVMSG.  Clang for comparison: 28 BDOS, 14 SNDMSG, 14 RCVMSG.  Drive-routing was the divergence; direct memory dump of `_cfgtbl` exposed the wrong server-id bytes.

(4) **Lessons captured in timeline + Phase 48 commit message**: when two compilers diverge on shared C source, dump the runtime data structures the foreign code reads from each compiler's binary as the FIRST diagnostic step.  Difference there finds the bug.  No difference there means the bug is in code paths and harder to find.  The Phase 48 hunt spent ~10 hours on ISR/stack/calling-convention investigation before pivoting to runtime-byte comparison; the bug was visible in 5 minutes once the right diagnostic was run.

(5) **Phase 48b cleanup** in `rc700-gensmedet@b7c6770`: retired the dead Python netboot harness.  Removed `netboot_server.py`, `sio_b_driver.py`, `mame_boot_test.lua`, `mame_sub_test.lua`, `mame_acid_test.lua`, `mame_smoke_dump.lua`, `mame_jt_probe.lua`, `testutil/acid.c`.  Removed targets `cpnos-mame`, `cpnos-netboot`, `cpnos-warmboot-test`, `cpnos-sub-test`, `conout-acid`, `cpnos-trace`, `cpnos-interactive`.  −1731 net lines.  All five removed targets had been broken since Path 6 (cpnos.com base shifts) and the SDCC port (different cfgtbl/BIOS-JT placements); their hardcoded pre-Path-6 addresses no longer matched the slave layout.  Issue #19 ("decide fate of legacy netboot.c / SERVER=proxy") priority-bumped — `SERVER=proxy` mode now has no working server.

(6) **Issue activity this session**: filed `ravn/z88dk#4` (compiler bug); filed `ravn/rc700-gensmedet#66` (SDCC SIO transport validation needed — only pio-irq tested in Phase 48); filed `ravn/rc700-gensmedet#67` (revert cfgtbl.c to macro form when z88dk#4 fixed); commented #19 priority bump; closed `ravn/rc700-gensmedet#60` (root cause writeup); closed `ravn/rc700-gensmedet#38` (superseded by polypascal-test PASS).

**Net session 48**: SDCC pio-irq slave at parity with clang for end-to-end test (first time ever); compiler bug found, workaround applied, upstream issue filed; ~50% reduction in dead Makefile harness.
...
(12) **Memory-map gap analysis (2026-05-07 evening) — initial 0xFFD0 IVT plan was WRONG; revised 2026-05-08**.  Display memory is hardware-mapped 0xF800..0xFFCF (2000 chars, 80×25), leaving 0xFFD0..0xFFFF = **48 B of scratch RAM** at the top of the address space (frame counter at 0xFFFC..0xFFFF + 44 B free).  Initial plan was to host the IVT there with I=0xFF — **this is unsafe**: under IM 2 the I register fixes a 256 B page, and the CPU reads its ISR pointer from anywhere in that page based on the device-supplied vector low byte.  With I=0xFF the page is 0xFF00..0xFFFF, of which 0xFF00..0xFFCF IS on-screen character RAM — any spurious or misprogrammed vector with low byte < 0xD0 jumps to a pointer composed of screen bytes.  All pages I=0xF8..0xFF are similarly forbidden because 0xF800..0xFFCF is display.  Valid IVT pages are those whose entire 256 B range is owned by the slave AND not display: practical candidates are **0xEC00 / 0xED00** (BSS scratch — carve 36 B + page-alignment slack from BSS, shift other BSS up; **zero resident cost**; clang would also recover 36 B currently used at `__ivt_start = 0xF500`) or **0xF500** (clang's current location, inside resident — costs resident bytes; the original session-47 SDCC blocker).  See `project_rc702_ivt_page_constraint.md` memory entry.  The 44 B at 0xFFD0..0xFFFB stays as scratch RAM (frame counter, future use), NOT IVT.
