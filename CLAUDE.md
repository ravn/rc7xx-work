...
- **cpnos-in-c PROM1-only line program** (production target): clang × {PIO+SIO} dual — compressed **2015 B / 2048 B (33 B free)** re-measured 2026-06-15 (was 2030 B / 18 B free 2026-06-08; the −15 B is Step 0 DMA-reload strip in `9592c2d` minus the +8 B Step 1 INIR scaffold + accumulated backend gains; same direction as rcbios's 5890→5462 drop).  polypascal-test PASS on BOTH transports (pio-irq AND sio; the harness now sets the SW1 S03 DIP from `$TRANSPORT`).  SDCC: **2151 B** / 4 KB padded, MAME-only (needs PROMCFG=2, 2732 4 KB); polypascal-test PASS.  Build: `cd cpnos-in-c && make prom1-lineprog COMPILER={clang,sdcc}`.  Shared init.c / resident.c; compiler-specific cold-init via bootstrap.s (clang) vs bootstrap.asm (SDCC).
...
- Z80 lit suite: **155 PASS + 6 XFAIL**, CI green.  Two-tier CI on llvm-z80 main: `build-and-lit` (lit) + `runtime-tests` (test-runner via z88dk-ticks); production verifier-clean enforced by `z80-utils/test-runner/scripts/verify-production.sh`.  #212 (the O0 residual of #197) CLOSED 2026-06-22 — narrowed NeedSaveHL in GR8 large-offset expanders, the O0 clang testcase corpus is now verifier-clean too.
