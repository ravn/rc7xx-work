...
## Current Sizes (2026-05-21, post-session-73p: #172 A-pin conservative scope + #166 ADD_HL_rr/ADD16_tied dead-ends documented + #173 NEW filed for 8-bit BSS spill peephole)
...
- **cpnos-in-c PROM1-only line program (session 73m snapshot)**: BOTH clang and SDCC build, boot, pass `cpnos-polypascal-test`.
    * **clang × {PIO+SIO} dual:** **2030 B / 2048 B (18 B free)** at HEAD with #168 SimplifyCFG cost-gate applied (llvm-z80 `cd2a2ace8754`).  +5 B vs pre-#168 baseline (2025); the cost-gate fix prevents the branchless-XOR pattern in `aes_mc_inv`-style code and shrinks `aes_subBytes` etc., net +5 B on cpnos (still under the 2 KB hard cap with comfortable margin).  Production target.  polypascal-test PASS **51.69 s**.
...
- **PROM budget watch**: cpnos PROM1 at 18 B free (2030 / 2048 B post-#168).  Any further cpnos growth still tight -- 2 KB cap is hard.  Next compiler shrink lever queued: #173 (8-bit BSS spill peephole, estimated 100-200 B AES yield with proportional cpnos savings).
...
- Z80 lit suite: **104 PASS + 3 XFAIL (107 total)**, CI green
