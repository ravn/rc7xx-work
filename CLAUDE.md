...
- autoload PROM (clang, ZX0-compressed): **1652 B / 2048 B (396 B free)** — measured 2026-05-31.  Banner: `RC700 ROA375 CL <date> <hash>/<user>`.  Hard-capped at 2 KB (no A11 bridge on user's hardware — memory rule `project_rc702_2kb_prom_hard_limit`).
- BIOS: clang **5897 B** vs SDCC **6091 B** (measured 2026-05-30; the #150 close later added +4 B in `_bg_clear_from`, accepted on the uncapped target).  MAME boot verified (signon + A> + disk ERR=0 across 77 tracks).
- **cpnos-in-c PROM1-only line program** (production target): clang × {PIO+SIO} dual — payload **2012 B**, compressed **2023 B / 2048 B (25 B free)** post-#150 (2026-05-31); polypascal-test PASS on BOTH transports (pio-irq AND sio; the harness now sets the SW1 S03 DIP from `$TRANSPORT`).  SDCC: **2151 B** / 4 KB padded, MAME-only (needs PROMCFG=2, 2732 4 KB); polypascal-test PASS.  Build: `cd cpnos-in-c && make prom1-lineprog COMPILER={clang,sdcc}`.  Shared init.c / resident.c; compiler-specific cold-init via bootstrap.s (clang) vs bootstrap.asm (SDCC).
- cpnos-in-c resident (clang, PIO transport): **clang 2004 B / SDCC 2120 B** raw .payload non-padding.
- AES-256 corpus (rc700-gensmedet/tasks/aes256-corpus): `09_Oz_prod_like` clang **−22 % size, −11 % faster** vs zsdcc; all 13 configs PASS verifier and beat SDCC on speed by 4.8–11 %.  See `llvm-z80/tasks/session73p-phase1-summary.md`.
