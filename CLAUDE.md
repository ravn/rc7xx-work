...
- **cpnos-in-c PROM1-only line program (session 73j-end)**: BOTH clang and SDCC now build, boot, pass polypascal-test.
    * **clang × {PIO+SIO} dual:** 2027 B / 2048 B (21 B free), fits a physical 2 KB PROM1 socket.  Production target.  polypascal-test PASS 50.99 s.
    * **SDCC × {PIO+SIO} dual:** 2246 B / 4 KB padded.  Needs PROMCFG=2 (2732 4 KB) in MAME; doesn't fit 2 KB physical socket yet (~168 B over).  polypascal-test PASS 49.81 s.
