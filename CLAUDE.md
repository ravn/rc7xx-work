...
## Current Sizes (2026-05-17, post-session-73j-late: locale tag on cpnos.img stamp + col-80 MAME fix + 2-PROM PARKED)

- autoload PROM (clang, ZX0-compressed): **1667 B / 2048 B (381 B free)** — +6 B over session-73j baseline for the cold-boot screen-clear (memset dspstr 0x20×2000 before banner copy, kills the 0x00-Danish-glyph background flicker visible frame-by-frame in MP4 captures).  Banner restored to descriptive form: `RC700 ROA375 CL <date> <hash>/<user>`.  Hard-capped at 2 KB (no A11 bridge on user's hardware -- memory rule `project_rc702_2kb_prom_hard_limit`).
...
- **cpnos-in-c PROM1-only line program (clang × {PIO+SIO} dual, session 73j-late)**: **2027 B / 2048 B (21 B free)** in a SINGLE 2 KB PROM1 socket.  Both `transport_pio.o` and `transport_sio.o` linked; SW1 bit 2 (S03) selects at cold-init via a 3-byte JP-NN trampoline pair in `.resident` patched by `install_transport()`.  PolyPascal-test PASS + ascii-print verified end-to-end.  Includes: pre-fill identity outcon + sentinel arm consolidated into `cpnos_cold_entry()` (C, shared by both compilers + both cold paths); locale tables (US-ASCII outcon from CONFI.COM + Danish inconv) installed from cpnos.img 384 B prefix at handoff.  Cost vs session-73j 2000 B baseline: +27 B for cpnos.img locale-tag-in-stamp + consolidated pre-init.  Build: `cd cpnos-in-c && make prom1-lineprog`.
