...
- **#150 CLOSED (2026-05-31, same session continuation, llvm-z80 `aee6f98`)** — the i16 EQ/NE HighByteZero path now extracts A via `sub_lo` instead of materialising the pair into HL.  The latent pio-irq miscompile that blocked it (sub-register liveness) was fixed by #156428/#210 (landed after #150 was filed); re-ran the original repro → **pio-irq cpnos polypascal PASSES**.  Win: **cpnos −8 B resident RAM** (payload 2020→2012; compressed 2023, under 2 KB cap), AES shrinks all configs (09_prod −3).  Oracle: lit 143+4, AES 13/13, test-runner −diff-opt −full clean, autoload neutral.  **BIOS +4 B** in `_bg_clear_from` — traced via `-debug-only=regalloc` to a benign greedy *inline-spill* of a 16-bit pair (register assignments unchanged; relaxing the incidental HL anchor under pressure flips one keep-resident to a BSS spill); accepted on the uncapped target.  **Finding (separate, NOT a clang bug — harness config gap):** the cpnos-polypascal-test `TRANSPORT=sio` mode does not actually validate SIO.  `make TRANSPORT=pio-irq prom1-lineprog` and `TRANSPORT=sio` produce a **byte-identical** prom1 (the prom1-lineprog payload is the single dual SW1-dispatched build; `TRANSPORT_NAME` only feeds the two-PROM server banner).  The runtime transport is chosen by `install_transport()` reading **SW1 bit 2 (S03)** at cold-init, and the polypascal test/lua **never sets the MAME DIP** — so `TRANSPORT=sio` runs the same binary in a config whose SW1/wiring isn't set for SIO (MAME wires SIO-A↔MP/M `:4002`).  Hence it "fails" identically with/without #150 (same binary) — not a clang codegen divergence (clang-pio works; SDCC-sio passes via the *different* PROMCFG=2/4 KB config that does set things up).  To truly test clang-SIO: set MAME SW1 S03=Off + matching wiring (the 4 KB path).  Marker-based instrumentation of the SIO path is blocked — enabling BOOT_MARK overflows the `.init` 640 B region by 61 B (the SIO build is at the size edge).
