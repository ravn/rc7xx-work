...
- **cpnos-in-c PROM1-only line program (clang × {PIO+SIO} dual, session 73j)**: **2000 B / 2048 B (48 B free)** in a SINGLE 2 KB PROM1 socket.  Both `transport_pio.o` and `transport_sio.o` linked; SW1 bit 2 (S03) selects at cold-init via a 3-byte JP-NN trampoline pair in `.resident` patched by `install_transport()`.  Default-PIO smoke-tested + cpnos-polypascal-test PASS (49.01s, ~1.8% faster vs pre-hot-path-reorder baseline).  Build: `cd cpnos-in-c && make prom1-lineprog`.  Cost vs PIO-only: +51 B net (SIO body + JT + patcher = +69 B, minus -19 B install_transport word-store rewrite + dead `#if 0` cleanup).
