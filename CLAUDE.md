...
- autoload PROM (clang, ZX0-compressed): **1660 B PROM / 2048 B cap (388 B free)** — re-measured 2026-07-01.  Boot gate PASS (`make floppy-boot-test` → `A>` on unpatched `SW1711-I8.imd`).  A temporary SIO-B polled-debug facility (added 2026-06-28 `a7a7293` while debugging a no-start) had dropped headroom to 139 B; **removed 2026-07-01** (ravn/rc700-gensmedet#118 closed), headroom back to the 388 B baseline.  A better debug path (gdb-z80 stub in `tasks/gdb-z80/`) is parked for later.  Banner: `RC700 ROA375 CL <date> <hash>/<user>`.  Hard-capped at 2 KB (no A11 bridge on user's hardware — memory rule `project_rc702_2kb_prom_hard_limit`).
