...
- **cpnos-in-c PROM1-only line program (session 73k snapshot)**: BOTH clang and SDCC build, boot, pass `cpnos-polypascal-test`.
    * **clang × {PIO+SIO} dual:** **2045 B / 2048 B (3 B free)** at HEAD with the session-73l SimplifyCFG patch applied (was 2027 B / 21 B free at session-73k snapshot; the +18 B drift is from llvm-z80 changes between 73k and now, NOT cpnos source -- verified by bisect across 5b0ae6a~1..HEAD which all produce identical 2045 B md5 `2596d8c1...` with current toolchain).  Production target.  polypascal-test PASS **51.33 s**.
