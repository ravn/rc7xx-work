...
- autoload PROM (clang, ZX0-compressed): **1481 B / 2048 B (567 B free)** — re-measured 2026-06-23 (was 1494 B 2026-06-08; −13 B from accumulated llvm-z80 backend gains incl. #239 + earlier).  Banner: `RC700 ROA375 CL <date> <hash>/<user>`.  Hard-capped at 2 KB (no A11 bridge on user's hardware — memory rule `project_rc702_2kb_prom_hard_limit`).
...
- **cpnos-in-c PROM1-only line program** (production target): clang × {PIO+SIO} dual — compressed **1987 B / 2048 B (61 B free)** re-measured 2026-06-23 (was 2015 B / 33 B free 2026-06-15; −28 B payload + −16 B compressed from accumulated llvm-z80 backend gains incl. #239 + earlier).  polypascal-test PASS on BOTH transports (pio-irq AND sio; the harness now sets the SW1 S03 DIP from `$TRANSPORT`).  SDCC: **2151 B** / 4 KB padded, MAME-only (needs PROMCFG=2, 2732 4 KB); polypascal-test PASS.  Build: `cd cpnos-in-c && make prom1-lineprog COMPILER={clang,sdcc}`.  Shared init.c / resident.c; compiler-specific cold-init via bootstrap.s (clang) vs bootstrap.asm (SDCC).
