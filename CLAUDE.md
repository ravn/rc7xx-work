...
- **cpnos-in-c PROM1-only line program (session 73k snapshot)**: BOTH clang and SDCC build, boot, pass `cpnos-polypascal-test`.
    * **clang × {PIO+SIO} dual:** **2048 B / 2048 B (0 B free)** at HEAD, fits a physical 2 KB PROM1 socket but at the hard cap (was 2027 B / 21 B free at session-73k snapshot; drift across May-18 commits + 73l SimplifyCFG patch; see timeline 73l-fix-bisect for the breakdown).  Production target.  polypascal-test PASS **51.33 s**.
