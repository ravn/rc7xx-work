...
- **cpnos-in-c PROM1-only line program (session 73k snapshot)**: BOTH clang and SDCC build, boot, pass `cpnos-polypascal-test`.
    * **clang × {PIO+SIO} dual:** **2022 B / 2048 B (26 B free)** at HEAD with the session-73l SimplifyCFG patch reverted (md5 `de180b83...`, deterministic mod ZX0/date-string noise of ~3 B).  Matches session-73k baseline (2027 / 21 free).  The +23 B "drift" investigated mid-session was entirely the SimplifyCFG patch (reverted in llvm-z80 commit `4336a23e81b5`).  Production target.  polypascal-test PASS **51.33 s**.
