...
## Six source-level variants tested
...
1. The `if (atb == x)` icmp's outside-graph wide operand is unblocked
   by **ANSI prototype** (parameter `x` is i8, no i16 carriage).
...
**v4 is the minimal "make the bug disappear" source form.**  It produces
48 B (matches the bug 4 fix's output) on stock LLVM.

**v1 alone (ANSI prototype) saves 8 B (62 vs 70)** by unblocking just
