...
| 4 | TruncInstCombine outside-user bail | Re-verified on de59f9ed 2026-06-07 (both fns stay i32). NOTE: upstream has NO icmp-const allowlist at all — any outside user of a non-ext node aborts (TruncInstCombine.cpp:276-283); the "allowlist" was our fork's prior extension, so the draft targets the all-or-nothing bail. Missed-optimization framing. **STAGED as ravn/llvm-z80#219, HOLD** (2026-06-07 AVR triage): micro-shapes equalise on AVR; the gf_log-scale claim is untested there AND our Z80 numbers are contaminated by the icmp-narrow soundness gate until the gate-fix + re-measure (branch `icmp-narrow-soundness-tests`). Repro: `issue-165-trunc-outside-user.ll`. **2026-06-15 thorough investigation → concrete unpark plan at `plan-bug4-unpark-2026-06-15.md`.**  Key findings: (1) the bug 4 PRODUCTION FIX already ships in fork main — commits `fa1606f34c6b` (#160 sound version) + `c4f52eb17a76` (#165 and-mask path); production wins on rcbios/cpnos already accrued.  (2) The HOLD is on the *upstream filing*, not the implementation.  (3) "Gate fix" in the original note conflated bug 4 with the CVP-strips-narrowness-marker issue (AES K&R +51 % speed gap, separate concern, ACCEPTED per `tasks/memory/project_aes_kr_speed_gap_accepted.md`).  (4) The remaining blockers are an AVR gf_log-scale oracle (Phase 1, ~1-2 h) + a clean Z80 measurement isolating bug 4 from CVP-strip (Phase 2, ~1 h) + RFC update + filing verdict.  Total ~3-4 h, all measurement and writing — no code changes.  Plan supersedes the retired TTI-hook proposal. |
