...
| 3 | SimplifyCFG foldTwoEntryPHINode | **DROPPED from upstream queue 2026-06-10** per AVR triage 2026-06-07: no in-tree target overrides `getPredictableBranchThreshold().isZero()` (`grep` confirms), the isZero() code path has zero in-tree constituency.  AVR's select=branch=6-instr equalisation shows the −16 B Z80 win was partly a measurement of our expensive select lowering, not a generic-LLVM miss.  Reasoning is sound (the `extractBranchWeights`-gated threshold consultation IS dead code on non-PGO targets), but without an in-tree victim upstream reviewers will reasonably close it.  Repro `bug3-twoentry-phi-no-pgo.ll` and `draft-bug3.md` kept as fork-internal knowledge; local SimplifyCFG cost-gate (commit `cd2a2ace8754`) is the right answer for our needs. |
