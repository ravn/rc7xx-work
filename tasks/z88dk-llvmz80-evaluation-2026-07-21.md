...
| `printf("%f")` | **Fixed 2026-07-22** | nanoprintf-backed printf family in `llvmz80-softfloat/src/npf_printf.c`, packaged in `softfloat_cpm_z80.lib`, byte-identical to glibc (`tests/ft_printf`).  **Transparent opt-in**: compile with `-D__LLVMZ80_IEEE_PRINTF` and **stock** `printf/fprintf/sprintf/snprintf` route to the shim (`printf("%f",x)` just works — all specifiers).  Opt-in (not default) so integer-only printf programs, which may not link the softfloat archive, pay no ~3 KB nanoprintf cost.  Without the define, or via explicit `__llvmz80_printf`, the shim is still callable.  Test: `test/clang/runtime_printf_ieee.sh`.  Unblocked by fixing a clang-z80 jump-table off-by-one that made nanoprintf's `%x` fail (`llvm-z80/tasks/bug-jumptable-upper-bound-offbyone.md`). |
| 64-bit (`long long`/`int64_t`) **global initializers** | **Fixed** 2026-08-05 (ravn/z88dk#27) — split in the backend | clang used to emit a 64-bit initializer as GNU 8-byte `.quad`; z88dk z80asm has no 8-byte directive (`DEFQ` is 4 bytes) and copt is a 32-bit-`int` engine (verified — cannot split it), so the value was truncated to 32 bits. **Fix:** `ravn/llvm-z80` `Z80MCAsmInfo` now leaves `Data64bitsDirective = nullptr`, so `MCAsmStreamer` splits each 8-byte value into two little-endian `.long` halves — no `.quad` reaches the bridge, and copt's existing `.long -> DEFQ` (4-byte) rule lowers each half. Safe because a symbolic 8-byte value cannot be formed on this target (casting a 16-bit address to a 64-bit initializer is rejected as non-constant; all emitted `.quad` are absolute). The external `lib/llvmz80/splitquad.pl` pre-pass was **deleted** (realising @ravn's preference to avoid an external program). Lit test `llvm/test/CodeGen/Z80/quad-init-split-27.ll` (`.quad` forbidden); e2e regression `test/clang/runtime_quadinit.{c,sh}` passes with the pre-pass gone (red/green verified). Design writeup: `z88dk/tasks/design-quad-init-64bit-2026-08-05.md`. |
