...
| 64-bit (`long long`/`int64_t`) **global initializers** | **Works via pre-pass — approach under review** (ravn/z88dk#27) | clang emits a 64-bit initializer as GNU 8-byte `.quad`; z88dk z80asm has no 8-byte directive (`DEFQ` is 4 bytes) and copt is a 32-bit-`int` engine (verified — cannot split it). Currently handled by the `lib/llvmz80/splitquad.pl` pre-pass (one of 3 external perl pre-passes in `bridge_postproc.sh`), which splits each `.quad` into two `.long` halves; regression test `test/clang/runtime_quadinit.{c,sh}`. NOTE: z88dk's own [Datatypes wiki](https://github.com/z88dk/z88dk/wiki/Datatypes) documents `long long`/`int64_t` as **fully supported** (64-bit) for sccz80/sdcc, so this is **not** a pre-existing z88dk limitation — it is llvmz80-bridge-specific and absent from the (ez80-clang-only) Clang-support wiki page. @ravn prefers dropping the external program in favour of a documented limitation + a clang warning; the right resolution (external pre-pass vs limitation+warning vs native 8-byte z80asm directive) is **deferred to the z88dk maintainers**. Design writeup: `z88dk/tasks/design-quad-init-64bit-2026-08-05.md`. |
