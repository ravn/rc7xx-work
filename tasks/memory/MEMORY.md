...
- [z88dk newlib signed % drops sign (stale prebuilt lib)](reference_newlib_signed_mod_z88dk_bug.md) — GENUINE z88dk bug (not clang): 8/16-bit signed `%` returns `|a%b|` on newlib; stock sccz80/sdcc reproduce it. Stale libs predate upstream fix af5630797c (2026-06-28). Bridge matches z88dk (parity OK for now); report `BUG_newlib_signed_mod.md` + xfail test `xfail_signed_mod.*`.
