...
- [stdcbench 0.8 on CP/M-86 via Watcom→DR C bridge](reference_rc759_mame_c_verification.md) — `stdcbench-cpm86.sh -m s` builds all 16 modules clean (0 warnings) → **c90base=7 c90lib=5 final=12** on the Unicorn runner (only it emulates the RC759 XIOS Int 28h timer stdcbench self-times with; emu2/MAME can't). Blocker was port `inc/stdlib.h` declaring malloc/calloc/realloc `void*` vs target `_preincl.h` `char*` (E1062) — reconciled to char* (register-identical DR C ABI). LARGE model still hangs (documented). Lives in open-watcom-v2 submodule.
