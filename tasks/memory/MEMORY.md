...
- [newlib clang route needs integer helper libcalls](reference_newlib_integer_helper_gap.md) — Phase C landed `-clib=newlib_iy/_ix` (`-compiler=llvmz80`) + compiler.h `__LLVMZ80` mapping (closes the ucpp `__smallc`/`__attribute__` gap). But clang emits gcc-style `__mulhi3`/`__divsi3`/`__divmodsi4` etc. absent from newlib (`-nostdlib`); classic bridge needs `config_private.inc`+`l_*` cores. Next blocker; qsort/intdiv/long SKIP on it.
