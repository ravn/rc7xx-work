...
- [newlib clang remaining gaps — disk FILE* (#34) + variadic %f (#35)](reference_newlib_remaining_gaps_file_printf.md) — the two genuine newlib_iy skips after Phase C; both root-caused + filed. #34 FILE* = CP/M newlib target has no file-open driver (asm_target_open_p1/p2 undefined; compiler-independent). #35 %f = clang IEEE-754 double not formatted by newlib variadic printf (no __LLVMZ80_IEEE_PRINTF route for newlib; Phase D). classic printf_ieee skip + xfail_tmpfile are NOT product gaps.
