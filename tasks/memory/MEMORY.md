...
- [RC759 MAME C-program verification harness](reference_rc759_mame_c_verification.md) — run mame-tests/run-mame.sh (build→cpmtools disk swap→boot→snapshot); regnecentralend MUST have FDC fix 59b21dc1312, rm nvram to autoboot, ~290s boot, install CMD as menu.cmd on mandel.img. Achieved 17/17 PASS int+file-I/O (not emu2). Oracle = snapshot PNG.
- [Watcom→DR C bridge: long accumulated across a loop not written back](reference_watcom_drc_long_loop_bug.md) — `-ecc -0 -ml`: `for(i..){r=r*10;}` yields 1 not 10000 (store-back/reg-alloc bug, __I4M clobbers the pair); single long ops outside loops are correct. Keep loop-carried arithmetic in int. CONFIRMED in MAME (hangs, 45 identical snaps); next bwdis the missing store + file it.
