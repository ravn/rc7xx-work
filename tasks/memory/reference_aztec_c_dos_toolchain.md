...
- **aztec34 = Aztec C86 3.40a/b (K&R)** — FULLY installed (bin+lib+include).
  This is the working DOS compiler today.
- **aztec42 = Aztec C86 4.2 / 4.10d (almost-ANSI, the NEWEST — user prefers it)**
  — only `share/aztec42/src` archives are present; **binaries NOT unpacked**.
  Getting 4.2 needs a fetch+build: `src/fetch/aztec42` pulls
  `https://www.aztecmuseum.ca/AztecC86.zip`, then `src/fetch/buildaztec42`
  unpacks/patches/builds libs (`src/aztec42-patch/`).

## Working DOS build — the one-liner recipe (VERIFIED)
K&R source (3.40 is NOT ANSI — `int main(void){...}` is REJECTED; use old style):
```c
#include <stdio.h>
main(){ printf("Hej fra Aztec C paa DOS\n"); return 0; }
```
```sh
cd open-watcom-v2/contrib/ravn/cpm86-crossdev
export PATH="$PWD/bin:$PATH"
aztec34_cc  -I. t.c            # -> t.o   (C -> asm -> Aztec object; K&R)
aztec34_link -o t.exe t.o -lc  # -> t.exe (real "MS-DOS executable")
...
`aztec34_obd file.o` disassembles the Aztec object. Sample `printf("Hello")`
body: `55 8b ec` (push bp; mov bp,sp) / `b8 .. 50` (mov ax,&str; push) /
`e8 ..` (call printf_) / `83 c4 02` (add sp,2) / `33 c0 8b e5 5d c3`
(xor ax,ax; mov sp,bp; pop bp; ret) — clean standard 8086.
