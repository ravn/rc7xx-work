...
**How to apply:**
- Toolchain:
  - sonnyboy: upstream `~/llvm-upstream/llvm-project/build` built with `LLVM_TARGETS_TO_BUILD="X86;AArch64;AVR;MSP430"`.
  - macbook (added 2026-06-07): the llvm-z80 fork build at `llvm-z80/build-macos/bin/` now also targets AVR+MSP430 alongside Z80/SM83 (`LLVM_TARGETS_TO_BUILD="AVR;MSP430"`, Z80/SM83 stay in `LLVM_EXPERIMENTAL_TARGETS_TO_BUILD`). Caveat: AVR codegen here runs through the llvm-z80 middle-end fork, not pure upstream — fine for shape/density comparison, file pure-upstream evidence from sonnyboy.
  - Invocation: `clang --target=avr -mmcu=atmega328p` / `llc -mtriple=avr`.
