...
description: On z80, double is 32-bit binary32 (float32-math32, #277). The 64-bit llvmz80-softfloat/ closure has been RETIRED (#44, DONE).
metadata:
  type: project
---
...
Consequence — user directive (2026-08-06), tracked in **ravn/z88dk#44**: **clean up / retire `llvmz80-softfloat/`.** DONE (see status above).
It was a *64-bit* Berkeley-SoftFloat closure (`__floatsidf`/`f64`/`i2d`, IEEE
...
no longer emits `df` libcalls to bridge to. Removed in the cleanup:
the `llvmz80-softfloat/` tree + its built `softfloat_cpm_z80.lib`
(`LLVMZ80RTLIB`), its `tests/ft_*.c`, the retired nanoprintf `runtime_printf_ieee`
test, and the 64-bit-double / IEEE-printf paragraphs in CLAUDE.md.
...
**How it was applied:** treat 64-bit double on z80 as not-a-goal. The
`.rodata.cstN` regression guard (ravn/z88dk#30) was already relocated
ABI-independently in-tree as `z88dk/test/clang/runtime_rodata_cstn.{c,sh}`
(int/long arrays → `.rodata.cst4/8/16/32`, volatile-indexed, checked across
O0-O3) — no 64-bit-double construct. See
