...
- **[RC702 bank2h PROM mirror](feedback_rc702_bank2h_mirror.md) — HARD: 0x2800..0x2FFF is NOT plain RAM; it's the bank2h PROM1-extended mirror. Writes vanish, reads return PROM1 bytes. Use 0x1000..0x1FFF or 0x3000..0xF7FF for BSS before PROM-disable. Caught session 73e (cpnos-in-asm rx_frame_buf).**
- **[Grep mem_map before BSS literal](feedback_grep_memmap_before_bss.md) — HARD: before allocating BSS at a literal address on an embedded target, grep the MAME (or other emulator) driver's mem_map for that range. 10 seconds with grep saves debug cycles. Generalizes [[feedback-rc702-bank2h-mirror]]**
- **[Slave RAM state outside TPA](feedback_slave_state_outside_tpa.md) — HARD: slave-side cursor/ring/BSS at 0x0100..BDOS-1 is INSIDE TPA -- CP/M programs (PPAS, PRIMES.PAS etc.) silently overwrite. PIN STATE INSIDE THE SNIOS RESERVED AREA (0xED00..0xF7FF on RC702); 0xEC00 is BDOS, NOT inside snios. Caught session 73g: dot_row at 0x4003 was clobbered by PPAS stack, cursor jumped 20 rows mid-banner.**
- **[Phase-boundary state-address audit](feedback_state_address_phase_audit.md) — HARD: when a piece of state changes lifecycle (prom-init-only -> post-handoff RAM-resident), RE-AUDIT its address vs. the destination phase's memory map. Safe pre-handoff (no TPA programs) != safe post-handoff. Generalizes [[feedback-slave-state-outside-tpa]] + [[feedback-rc702-bank2h-mirror]]. Caught session 73g after rx_frame_buf made the same class of mistake in session 73e.**
...
- **[Diff binaries before blaming codegen](feedback_diff_binaries_before_blaming_codegen.md) — HARD: when two compiler configurations "behave differently" at runtime, `cmp -l a b` FIRST.  Byte-identical binaries can't have a codegen miscompile — the failure is environmental (stale daemons, harness flake).  Don't bisect attribute sets without this check.**
- **[Verify writes before chasing reads](feedback_verify_writes_before_chasing_reads.md) — HARD: when a buffer reads back wrong bytes, FIRST check the write succeeded (instrument right before and right after the store). Only THEN investigate input sources. Avoided this in session 73e and spent 4 iterations on red-herring loopback/baud/echo hypotheses.**
- **[Recognize ROM-shadow byte patterns](feedback_recognize_rom_shadow_patterns.md) — HARD: when buffer/port reads produce structured-looking "wrong" bytes, `xxd build/prom*.bin | head` and grep for the sequence BEFORE wire-level hypotheses. If the bytes match in-tree ROM contents at any offset, the bug is memory-map / mirror / unmapped-read, not transport. Session 73e: 08 20 20 52 43 37 30 was literally prom1.bin[0..6].**
- **[No taps inside polled-RX hot path](feedback_no_taps_in_polled_rx.md) — HARD: per-byte blocking debug TX inside a polled receive routine overruns the Z80 SIO 3-deep RX FIFO and drops bytes silently — masquerades as "master skipped a byte". Use per-frame markers, or ring-buffer the trace and flush after EOT. Session 73f #75 -- 1 FAIL/5 PASS -> 6/6 PASS by removing `>XX`/`<XX` from `snios_sio_a_rx_to`/`_tx_a`.**
